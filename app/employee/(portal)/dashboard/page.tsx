...
export default async function EmployeeDashboardPage() {
  ...
  const [stats, activities, announcements, notifications, expenseSummary, approvedPurchasesRes] = await Promise.all([
  ...
  <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-emerald-600 text-xl font-bold text-white shadow-xs shrink-0">
    {initials}
            ...
            <div>
              ...
              <h1 className="mt-1 text-xl md:text-2xl font-extrabold tracking-tight text-slate-900">
                Welcome back, {firstName}
              </h1>

              <p className="text-xs text-slate-500 mt-0.5">
                <span className="font-semibold text-slate-700">{profile.designation}</span> &bull; Dept: <span className="font-medium text-slate-600">{profile.department}</span>
              </p>
            </div>
          </div>
          ...
          <div className="shrink-0 flex flex-col items-start md:items-end gap-1 text-xs font-medium text-slate-500 pt-2 md:pt-0 border-t md:border-t-0 border-slate-100">
            ...
            <span className="flex items-center gap-1 text-slate-500 text-xs">
              Status: <span className="text-emerald-600 font-semibold flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />Active</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3 items-start">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-xs">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Your approved personal sales revenue performance over 6 months</p>
              </div>
              <Link
                href="/employee/sales"
                className="text-xs text-emerald-600 hover:text-emerald-700 flex items-center gap-1 font-bold group"
              >
                My Sales <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-0.5" />
              </Link>
            </div>
          ...
          {/* 4. Monthly Expense Overview (placed directly beneath Sales Revenue) */}
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Commonly accessed employee portals</p>
            </div>
            ...
            <div className="mt-4 grid gap-3 grid-cols-2 sm:grid-cols-4">
              ...
              >
                ...
                <span className="mt-2.5 text-xs font-bold text-slate-800 group-hover:text-emerald-700">Apply Leave</span>
                <span className="mt-0.5 text-[10px] text-slate-500">Request time off</span>
              </Link>
              ...
              >
                ...
                <span className="mt-2.5 text-xs font-bold text-slate-800 group-hover:text-blue-700">Submit Expense</span>
                <span className="mt-0.5 text-[10px] text-slate-500">Reimbursement claims</span>
              </Link>
              ...
              >
                ...
                <span className="mt-2.5 text-xs font-bold text-slate-800 group-hover:text-violet-700">My Tasks</span>
                <span className="mt-0.5 text-[10px] text-slate-500">Track assigned work</span>
              </Link>
              ...
              >
                ...
                <span className="mt-2.5 text-xs font-bold text-slate-800 group-hover:text-amber-700">Incentives</span>
                <span className="mt-0.5 text-[10px] text-slate-500">View bonus claims</span>
              </Link>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Your latest update log</p>
            </div>
        ...
        <div className="space-y-6">
          {/* Quick Profile Overview Link */}
          <Link
            href="/employee/profile"
            className="group flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white p-5 shadow-xs transition-all hover:border-emerald-300 hover:shadow-emerald-50/50"
          >
            <div className="flex items-center gap-3.5">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100 group-hover:scale-105 transition-transform">
                <User className="h-5 w-5" />
              </div>
              <div>
                <p className="text-sm font-bold text-slate-900 group-hover:text-emerald-700 transition-colors">My Profile</p>
                <p className="text-xs text-slate-500">View & edit profile details</p>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {publishedAnnouncements.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1.5">No company announcements</p>
                </div>
              ) : (
